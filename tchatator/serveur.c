...
int main(int argc, char *argv[]) {
    ...
    while (1)  // while(1) pour garder le serveur allume malgre que le client quitte
    {
        ...
        if (!is_member_banned(conn, user_info->id_user, txt_log, 2050)) {
            ...
            send(cnx, "\033[31m-> Erreur : Vous êtes bannis.\33[0m\n", strlen("\033[31m-> Erreur : Vous êtes bannis.\33[0m\n"), 0);
            fprintf(stderr, "\033[31m%s\033[0m\n", txt_log);
        ...
        if (strcmp(user_info->new_user, "ADMIN") !=0) {
            snprintf(query, sizeof(query), "DELETE FROM tripenarvor._token WHERE token='%s'", user_info->token);
            if (!execute_query(conn, query, api_key, client_ip)) {
                printf("\033[31m-> Erreur suppression token : %s\033[0m\n", PQerrorMessage(conn));
            }
        }
    }
